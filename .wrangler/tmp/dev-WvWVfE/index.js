...
// .wrangler/tmp/bundle-S8vQVp/middleware-loader.entry.ts
init_modules_watch_stub();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_process();
init_performance2();

// .wrangler/tmp/bundle-S8vQVp/middleware-insertion-facade.js
init_modules_watch_stub();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_process();
init_performance2();
  ...
  return (context, next2) => {
    ...
    async function dispatch(i) {
      ...
      if (middleware2[i]) {
        ...
        handler = i === middleware2.length && next2 || void 0;
      }
      if (handler) {
        try {
...
var logger = /* @__PURE__ */ __name((fn = console.log) => {
  return /* @__PURE__ */ __name(async function logger2(c, next2) {
...
// src/services/otakudesu.service.ts
init_modules_watch_stub();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_process();
init_performance2();
...
var hasOwn = (
  ...
  (_a4 = Object.hasOwn) !== null && _a4 !== void 0 ? _a4 : ((object, prop2) => Object.prototype.hasOwnProperty.call(object, prop2))
);
var rspace = /\s+/;
...
function filter3(match2) {
  var _a6;
  return this._make(filterArray(this.toArray(), match2, this.options.xmlMode, (_a6 = this._root) === null || _a6 === void 0 ? void 0 : _a6[0]));
}
...
// src/services/otakudesu.service.ts
var OtakudesuService = class {
  static {
    __name(this, "OtakudesuService");
  }
  ...
  parsePagination($2, itemsPerPage = 50) {
    const paginationEl = $2(".pagenavix, .pagination");
    const currentPageEl = paginationEl.find(".page-numbers.current");
    const currentPage = parseInt(currentPageEl.text()) || 1;
    const allPages = paginationEl.find(".page-numbers:not(.prev):not(.next)").map((_, el) => parseInt($2(el).text()) || 0).get().filter((n) => n > 0);
  ...
  async getHome() {
    const $2 = await this.fetchHtml(this.baseUrl);
    const ongoing = [];
    const complete = [];
    $2(".venz ul").first().find("li").each((_, el) => {
      const $el = $2(el);
      const title2 = $el.find(".jdlflm").text().trim();
    ...
    $2(".venz ul").last().find("li").each((_, el) => {
      const $el = $2(el);
      const title2 = $el.find(".jdlflm").text().trim();
      const link = $el.find(".thumb a").attr("href") || "";
      const poster = $el.find(".thumbz img").attr("src") || "";
  ...
  async getOngoing(page = 1) {
    ...
    const $2 = await this.fetchHtml(url);
    const anime = [];
    $2(".venz ul li, .rapi .detpost").each((_, el) => {
      const $el = $2(el);
      const title2 = $el.find(".jdlflm, .judul-anime").text().trim();
      const link = $el.find(".thumb a, a").first().attr("href") || "";
      const poster = $el.find(".thumbz img, img").first().attr("src") || $el.find(".thumbz img, img").first().attr("data-src") || "";
      const episode = $el.find(".epz").text().replace("Episode", "").trim();
      const releaseDay = $el.find(".epztipe").text().trim();
      const releaseDate = $el.find(".newnime").text().trim();
      if (title2 && link && link.includes("/anime/")) {
        anime.push({
          title: title2,
          slug: this.extractSlug(link),
          poster,
    ...
    const pagination = this.parsePagination($2);
  ...
  async getAnimeList() {
    const $2 = await this.fetchHtml(`${this.baseUrl}/anime-list/`);
    const list = {};
    $2(".bariskelom").each((_, el) => {
      const $el = $2(el);
      const letter = $el.find(".barispenz a").text().trim().toUpperCase() || "#";
      if (!list[letter]) {
        list[letter] = [];
      }
      $el.find(".jdlbar ul li a").each((_2, linkEl) => {
        const $link = $2(linkEl);
        const title2 = $link.text().trim();
  ...
  async getAnimeDetail(slug) {
    const $2 = await this.fetchHtml(`${this.baseUrl}/anime/${slug}/`);
    const title2 = $2(".jdlrx h1").text().trim().replace(/\s*\(Episode\s*\d+\s*[-–]\s*\d+\)\s*/i, "").replace(/(\s*(Subtitle Indonesia|Sub Indo))+$/i, "").trim();
    const poster = $2(".fotoanime img").attr("src") || "";
    if (!title2) {
    ...
    const infoMap = {};
    $2(".infozingle p").each((_, el) => {
      const text3 = $2(el).text();
      const [key, ...valueParts] = text3.split(":");
      if (key && valueParts.length) {
        infoMap[key.trim().toLowerCase()] = valueParts.join(":").trim();
      }
    });
    const genres = [];
    $2('.infozingle p span a[href*="/genres/"]').each((_, el) => {
      genres.push($2(el).text().trim());
    });
    const synopsis = $2(".sinopc").text().trim();
    const episodes = [];
    $2(".episodelist ul li").each((_, el) => {
      const $el = $2(el);
  ...
  async getEpisodeDetail(slug) {
    const $2 = await this.fetchHtml(`${this.baseUrl}/episode/${slug}/`);
    const title2 = $2(".posttl").text().trim();
    if (!title2) {
      const error = new Error(`Episode dengan slug '${slug}' tidak ditemukan`);
      error.statusCode = 404;
      throw error;
    }
    const animeLink = $2('.flir a[href*="/anime/"]').attr("href") || $2('.flir a:contains("See All")').attr("href") || "";
    const animeTitle = title2.replace(/\s*Episode\s*\d+.*$/i, "").trim();
    const prevLink = $2('.flir a:contains("Previous")').attr("href") || "";
    const nextLink = $2('.flir a:contains("Next")').attr("href") || "";
    const streamingUrl = $2("#embed_holder iframe").attr("src") || "";
    const streamingServers = [];
    const qualityMap = {};
    $2(".mirrorstream ul").each((_, ulEl) => {
      const $ul = $2(ulEl);
      ...
      $ul.find("li a[data-content]").each((_2, linkEl) => {
        const $link = $2(linkEl);
    ...
    const downloadLinks = [];
    $2(".download ul li").each((_, el) => {
      const $el = $2(el);
      const resolution = $el.find("strong").text().trim();
      const links = [];
      $el.find("a").each((_2, linkEl) => {
        const $link = $2(linkEl);
        const provider = $link.text().trim();
    ...
    return {
      ...
      prevEpisode: prevLink ? this.extractSlug(prevLink) : void 0,
      nextEpisode: nextLink ? this.extractSlug(nextLink) : void 0,
      streamingUrl: streamingUrl || void 0,
      streamingServers,
      downloadLinks
    };
  }
  async getGenres() {
    const $2 = await this.fetchHtml(`${this.baseUrl}/genre-list/`);
    const genres = [];
    $2(".genres li a").each((_, el) => {
      const $el = $2(el);
  ...
  async getAnimeByGenre(genre, page = 1) {
    ...
    const $2 = await this.fetchHtml(url);
    const anime = [];
    $2(".col-anime, .col-md-4, .venz li, .page .col-6").each((_, el) => {
      const $el = $2(el);
      const title2 = $el.find(".col-anime-title a, .jdlflm, .judul-anime, h3, h4").first().text().trim();
  ...
  async getSchedule() {
    const $2 = await this.fetchHtml(`${this.baseUrl}/jadwal-rilis/`);
    const schedule = [];
    const days = [
      "Senin",
      "Selasa",
    ...
    $2(".kglist321").each((index2, el) => {
      const $el = $2(el);
      const day = $el.find("h2").text().trim() || days[index2] || `Day ${index2 + 1}`;
      const animeList = [];
  ...
  async search(query) {
    const $2 = await this.fetchHtml(
      `${this.baseUrl}/?s=${encodeURIComponent(query)}&post_type=anime`
    );
    const anime = [];
    $2(".chivsrc li").each((_, el) => {
      const $el = $2(el);
      const title2 = $el.find("h2 a").text().trim();
      const link = $el.find("h2 a").attr("href") || "";
      const poster = $el.find("img").attr("src") || "";
      let rating = "";
      $el.find(".set").each((__, setEl) => {
        const text3 = $2(setEl).text();
        if (text3.includes("Rating")) {
          rating = text3.replace("Rating :", "").trim();
        }
      });
      if (title2 && link && link.includes("/anime/")) {
        anime.push({
  ...
  async resolveStreamingUrl(dataContent) {
    ...
    const html4 = atob(encodedHtml);
    const iframeMatch = html4.match(/src="([^"]+)"/);
...
var otakudesuService = new OtakudesuService();
...
\`\`\``,
  ...
  paths: {
    "/api/home": {
      get: {
        ...
        description: "Retrieves the latest ongoing and completed anime displayed on the homepage. Returns two arrays containing anime cards with basic information.",
        operationId: "getHome",
        responses: {
          "200": {
                    ...
                    ongoing: [
                      {
                        ...
                        poster: "https://otakudesu.best/wp-content/uploads/2025/10/One-Punch-Man-Season-3.jpg",
                        episode: "8",
                        releaseDay: "Thursday",
                        releaseDate: "05 Dec"
                      }
                    ...
                    complete: [
                      {
                        ...
                        poster: "https://otakudesu.best/wp-content/uploads/2024/10/Dandadan.jpg",
    ...
    "/api/ongoing": {
      get: {
        ...
        description: "Retrieves a paginated list of currently airing anime. Each page contains up to 50 anime entries.",
    ...
    "/api/complete": {
      get: {
        ...
        description: "Retrieves a paginated list of completed anime. Each page contains up to 50 anime entries.",
    ...
    "/api/anime-list": {
      get: {
        ...
        description: 'Retrieves all anime sorted alphabetically and grouped by first letter (A-Z). Includes a special "#" group for anime starting with numbers or symbols.',
    ...
    "/api/anime/{slug}": {
      get: {
        ...
        description: "Retrieves detailed information about a specific anime including synopsis, info (score, studio, duration, etc.), genres, and complete episode list.",
              ...
              "application/json": {
                ...
                example: {
                  ...
                  data: {
                    ...
                    synopsis: "The seemingly ordinary and unimpressive Saitama has a rather unique hobby: being a hero...",
                    poster: "https://otakudesu.best/wp-content/uploads/2025/10/One-Punch-Man-Season-3.jpg",
                    episodes: [
                      {
                        episode: "8",
    ...
    "/api/episode/{slug}": {
      get: {
        ...
        description: "Retrieves detailed information about a specific episode including streaming servers (with multiple quality options) and download links (with multiple providers per resolution).",
            ...
            content: {
              "application/json": {
                ...
                example: {
                  ...
                  message: "OK",
                  data: {
                    title: "One Punch Man Season 3 Episode 8 Subtitle Indonesia",
                    anime: {
                      title: "One Punch Man Season 3",
                      slug: "punch-man-s3-sub-indo"
                    },
                    ...
                    streamingServers: [
                      {
                        ...
                        servers: [
                          {
                            provider: "Streamwish",
                            dataContent: "eyJpZCI6MTkwNDQzLCJpIjowLCJxIjoiMzYwcCJ9"
                          },
                          {
                            provider: "Doodstream",
                            dataContent: "eyJpZCI6MTkwNDQzLCJpIjoxLCJxIjoiMzYwcCJ9"
                          }
                        ]
                      },
                      {
                        ...
                        servers: [
                          {
                            provider: "Streamwish",
                            dataContent: "eyJpZCI6MTkwNDQzLCJpIjowLCJxIjoiNDgwcCJ9",
                            isDefault: true
                          }
                        ]
                      },
                      {
                        ...
                        servers: [
                          {
                            provider: "Streamwish",
                            dataContent: "eyJpZCI6MTkwNDQzLCJpIjowLCJxIjoiNzIwcCJ9"
                          }
                        ]
                      }
                    ],
    ...
    "/api/genres/{genre}": {
      get: {
        ...
        description: "Retrieves a paginated list of anime filtered by the specified genre.",
                  ...
                  data: {
                    ...
                    anime: [
                      ...
                      },
                      {
                        ...
                        poster: "https://otakudesu.best/wp-content/uploads/2020/10/Jujutsu-Kaisen.jpg",
                        rating: "8.60"
                      }
                    ],
                    pagination: {
    ...
    "/api/schedule": {
      get: {
        ...
        description: "Retrieves the anime release schedule organized by day of the week (Monday to Sunday).",
    ...
    "/api/search": {
      get: {
        ...
        description: "Search for anime by keyword. Returns anime matching the search query.",
                  ...
                  data: {
                    anime: [
                      ...
                      },
                      {
                        ...
                        poster: "https://otakudesu.best/wp-content/uploads/2019/04/One-Punch-Man-Season-2-Sub-Indo.jpg",
                        rating: "7.67"
                      },
                      {
                        title: "One Punch Man BD Subtitle Indonesia",
                        slug: "one-punch-sub-indo",
                        poster: "https://otakudesu.best/wp-content/uploads/2019/02/One-Punch-Man-Sub-Indo.jpg",
                        rating: "8.75"
                      }
                    ]
                  },
    ...
    "/api/resolve-streaming": {
      post: {
        ...
        description: "Resolves the streaming player URL from the base64 encoded data-content found in streamingServers. Use this endpoint to get the actual video player iframe URL.",
            ...
            "application/json": {
              schema: {
                ...
                properties: {
                  dataContent: {
                    type: "string",
                    description: "Base64 encoded data-content from streamingServers response",
    ...
    "/api/resolve-streaming/{dataContent}": {
      get: {
        ...
        description: "Alternative GET endpoint for resolving streaming player URL. Useful for simple requests without a JSON body.",
  ...
  components: {
    schemas: {
      AnimeCard: {
        ...
        properties: {
          ...
          poster: {
            ...
            example: "https://otakudesu.best/wp-content/uploads/2025/10/One-Punch-Man-Season-3.jpg"
          },
          episode: {
            type: "string",
      ...
      EpisodeDetail: {
        type: "object",
        description: "Detailed episode information with streaming and download links",
        properties: {
          title: { type: "string" },
          anime: {
            type: "object",
...
function successResponse(c, data2, startTime) {
...
app.get("/api/home", async (c) => {
  ...
  const data2 = await otakudesuService.getHome();
  return successResponse(c, data2, startTime);
});
app.get("/api/ongoing", async (c) => {
  const startTime = Date.now();
  const page = parseInt(c.req.query("page") || "1");
  const data2 = await otakudesuService.getOngoing(page);
  return successResponse(c, data2, startTime);
});
app.get("/api/complete", async (c) => {
  const startTime = Date.now();
  const page = parseInt(c.req.query("page") || "1");
  const data2 = await otakudesuService.getComplete(page);
  return successResponse(c, data2, startTime);
});
...
app.get("/api/anime/:slug", async (c) => {
  ...
  const data2 = await otakudesuService.getAnimeDetail(slug);
  return successResponse(c, data2, startTime);
});
app.get("/api/episode/:slug", async (c) => {
  const startTime = Date.now();
  const slug = c.req.param("slug");
  const data2 = await otakudesuService.getEpisodeDetail(slug);
  return successResponse(c, data2, startTime);
});
...
app.get("/api/genres/:genre", async (c) => {
  ...
  const page = parseInt(c.req.query("page") || "1");
  const data2 = await otakudesuService.getAnimeByGenre(genre, page);
  return successResponse(c, data2, startTime);
});
...
app.post("/api/resolve-streaming", async (c) => {
  ...
  const data2 = await otakudesuService.resolveStreamingUrl(dataContent);
  return successResponse(c, data2, startTime);
});
